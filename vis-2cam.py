...
# stateful LSTM to process streaming video from a webcam....or two, and attempts to predict what
# it will see next at some small timestep into the future.
...
micsac_magnitude=0.0  # 0.0 for none 2.0 for single pixel.  (this introduces little shaking movements
...
video_height = 240
video_width = 320 # x2 for binoculaaaar
...
video_input_size = video_height * video_width * video_channels * 2
...
self_output_to_self_size = 2*(2 + coords_dim)  # the 2 represents the variables that allow the net to say
...
def get_weights_biases():
    ...
    weights = {
        ...
        'enc_3_h1' : tf.get_variable('enc_3_h1', shape=(self_output_size - 2*coords_dim, num_hidden_1), 
                                     initializer = tf.contrib.layers.xavier_initializer()),
...
def model_eval(vision_0, vision_minus_1, vision_minus_2, vision_minus_3, 
    ...
    updown_leftright_1 = tf.multiply(tf.nn.tanh(tf.slice(dec_1_layer_3_pre, [0, 0], [1, 2])), 2.0)
    updown_leftright_2 = tf.multiply(tf.nn.tanh(tf.slice(dec_1_layer_3_pre, [0, 2], [1, 2])), 2.0)
    # and this one gotta be sigmoied
    req_data = tf.nn.sigmoid(tf.slice(dec_1_layer_3_pre, [0, 4], [1, 13]))
    req_data_max = tf.argmax(req_data, axis=1)
    
    # and recombining  # addition of prev act doesnt take place here, its in _pre
    dec_1_layer_3 = tf.concat([updown_leftright_1, updown_leftright_2, req_data], 1)
    ...
    updown_leftright_data_1 = updown_leftright_1
    updown_leftright_data_2 = updown_leftright_2
    ...
    return (prediction, output_data, updown_leftright_data_1, updown_leftright_data_2, req_data_max)
...
def loss_fxn(prediction, vision_target, vision_0, step_number):
    ...
    with tf.name_scope('losses'):
        with tf.name_scope('pred_error'):
            ...
            pred_error = tf.reshape(
                            tf.reduce_mean(
                                tf.subtract(
                                    tf.add(
                                        tf.minimum(l2, tf.constant([0.5])), 
                                        tf.maximum(l1,  tf.constant([0.5]))
                                    ), 
                                    tf.constant([0.5]))), 
                          [])
        ...
        pearson_correlation = tf.divide(
                                  tf.tensordot(x_diff, y_diff, 1),
                                  tf.multiply(
                                      tf.sqrt(tf.reduce_sum(tf.square(x_diff))),
                                      tf.sqrt(tf.reduce_sum(tf.square(y_diff)))
                                  )
                              )
        ...
        loss = (pred_error + corr_loss) * (1.0) # +noise)
...
def main():
    ...
    cap1 = cv2.VideoCapture(webcam_1_index)
    cap1.set(3,full_video_height)
    cap1.set(4,full_video_width)   
    current_x_coord_1 = np.array([int((full_video_width-video_width)/2.0)]) # binok
    current_y_coord_1 = np.array([int((full_video_height-video_height)/2.0)])
    
    cap2 = cv2.VideoCapture(webcam_2_index)
    cap2.set(3,full_video_height)
    cap2.set(4,full_video_width)   
    current_x_coord_2 = np.array([int((full_video_width-video_width)/2.0)]) # binok
    current_y_coord_2 = np.array([int((full_video_height-video_height)/2.0)])
    ...
    with tf.Session() as sess:
        ...
        input_y_coord,
        updown_leftright,
        visual_data) = placeholder_inputs()
        ...
        v_target_data_1 = get_vid_frame(cap1)
        v_target_data_2 = get_vid_frame(cap2)
        v_target_dataset_1 = tf.data.Dataset.from_generator(lambda: v_target_data_1, tf.float32)
        v_target_dataset_2 = tf.data.Dataset.from_generator(lambda: v_target_data_2, tf.float32)
        vision_target_frame_1 = v_target_dataset_1.make_one_shot_iterator().get_next()
        vision_target_frame_2 = v_target_dataset_2.make_one_shot_iterator().get_next()
        
        (             prediction, 
                     output_data, 
         updown_leftright_data_1, 
         updown_leftright_data_2, 
                    req_data_max,) = model_eval(vision_0, 
                                                vision_minus_1, 
                                                vision_minus_2, 
                                                vision_minus_3, 
                                                vision_minus_4, 
        ...
        visual_small_op = reduce_vision_window(visual_data, 
                                                input_x_coord, 
                                                input_y_coord, 
                                                updown_leftright)
        ...
        for i in range(6+num_video_lag_steps):
            visual_data_out_1 = sess.run(vision_target_frame_1)
            visual_data_out_2 = sess.run(vision_target_frame_2)
            (visual_small_data_1, 
             new_x_coord_1, 
             new_y_coord_1) = sess.run(visual_small_op, 
                                     feed_dict={visual_data: visual_data_out_1,
                                                input_x_coord: current_x_coord_1,
                                                input_y_coord: current_y_coord_1,
                                                updown_leftright: np.array([[0,0]])})
            
            (visual_small_data_2, 
             new_x_coord_2, 
             new_y_coord_2) = sess.run(visual_small_op, 
                                     feed_dict={visual_data: visual_data_out_2,
                                                input_x_coord: current_x_coord_2,
                                                input_y_coord: current_y_coord_2,
                                                updown_leftright: np.array([[0,0]])})
            
            current_x_coord_1 = new_x_coord_1
            current_y_coord_1 = new_y_coord_1
            current_x_coord_2 = new_x_coord_2
            current_y_coord_2 = new_y_coord_2
            #visual_enq_data = np.concatenate((visual_small_data[0], current_x_coord, current_y_coord))
            visual_small_data_1 = np.reshape(visual_small_data_1[0], 
                                                (video_height, video_width, video_channels))
            visual_small_data_2 = np.reshape(visual_small_data_2[0], 
                                                (video_height, video_width, video_channels))
            
            visual_enq_data = np.concatenate((visual_small_data_1, visual_small_data_2), axis=1)
                                                
            visual_enq_data = visual_enq_data.flatten()
        ...
        sess.run(
            self_output_queue.enqueue(
                np.reshape(
                    np.append(
                        np.zeros((1, self_output_size-4)), 
                        [[current_x_coord_1, current_y_coord_1, current_x_coord_2, current_y_coord_2]] ),
                    (1, self_output_size)
                )
            )
        )
        ...
        while True:
            ...
            (                         _, 
                         prediction_out, 
                        output_data_out, 
            updown_leftright_data_out_1, 
            updown_leftright_data_out_2, 
                       req_data_max_out, 
                               loss_out, 
                        loss_pixels_out, 
            ...
            (visual_data_out_1, 
             visual_data_out_2) = sess.run([vision_target_frame_1, 
                                            vision_target_frame_2])
            ...
            (visual_small_data_1, 
             ...
             new_y_coord_1) = sess.run(visual_small_op, 
                                      feed_dict={visual_data: visual_data_out_1,
                                                input_x_coord: current_x_coord_1,
                                                input_y_coord: current_y_coord_1,
                                                updown_leftright: updown_leftright_data_out_1})
            (visual_small_data_2, 
             new_x_coord_2, 
             new_y_coord_2) = sess.run(visual_small_op, 
                                      feed_dict={visual_data: visual_data_out_2,
                                                input_x_coord: current_x_coord_2,
                                                input_y_coord: current_y_coord_2,
                                                updown_leftright: updown_leftright_data_out_2})
            ...
            so_enq_data = np.reshape(np.append(np.array(output_data_out), 
                                               [current_x_coord_1, 
                                                current_y_coord_1,
                                                current_x_coord_2, 
                                                current_y_coord_2]), 
            ...
            # display windows
            this_frame = np.resize(v_t_0_pos,(video_height, video_width*2, video_channels)).astype(np.uint8)
            pred_frame = np.resize(255.0*pred_pixels,(video_height, video_width*2, video_channels)).astype(np.uint8)
            loss_frame = np.resize(255.0*loss_pixels_out, (video_height, video_width*2, video_channels)).astype(np.uint8)
            #conf_frame = np.resize(255.0*conf_pixels,(video_height, video_width, video_channels)).astype(np.uint8)
            actu_frame = np.resize(v_t_target_pos,(video_height, video_width*2, video_channels)).astype(np.uint8)
